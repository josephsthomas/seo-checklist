...
export default function ReadabilityShareView() {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      const { useReadabilityExport } = await import('../../hooks/useReadabilityExport');
      // We can't use hooks outside components, so just dynamically import jsPDF
      const { default: jsPDF } = await import('jspdf');
      const doc = new jsPDF();
      doc.setFontSize(20);
      doc.text('AI Readability Report', 20, 30);
      doc.setFontSize(12);
      doc.text(`Score: ${sharedAnalysis.overallScore || 'N/A'}/100`, 20, 45);
      doc.text(`Grade: ${sharedAnalysis.grade || 'N/A'}`, 20, 55);
      if (sharedAnalysis.sourceUrl) {
        doc.text(`URL: ${sharedAnalysis.sourceUrl}`, 20, 65);
      }
      doc.text(`Date: ${formatDate(sharedAnalysis.analyzedAt)}`, 20, 75);
      doc.text('Generated by Content Strategy Portal', 20, 280);
      doc.save(`readability-shared-report.pdf`);
    } catch (err) {
